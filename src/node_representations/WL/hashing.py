...
class WeisfeilerLehman(TransformerMixin):
    ...
    def _relabel_graphs(self, X: List[ig.Graph], initial_relabeling: str='degrees'):
        num_unique_labels = 0
        preprocessed_graphs = []
        for i, g in enumerate(X):
            ...
            if not 'label' in x.vs.attribute_names():
                ...
                if initial_relabeling == 'ones':
                    x.vs['label'] = list(map(str, [1 for _ in x.vs.degree()]))
                else: #elif initial_relabeling == 'degrees':
    ...
    def fit_transform(self, 
                      ...
                      initial_relabeling: str='degrees'):
        self._label_sequences = [
            np.full((len(g.vs), num_iterations + 1), np.nan) for g in X
        ]
        ###
        assert initial_relabeling in ['ones', 'degrees'], 'Relabeling method not implemented!'
        X = self._relabel_graphs(X, initial_relabeling=initial_relabeling)
        ###
...
def computeNodeRepresentations(X, depth, initial_relabeling):
    '''
    ...
    '''
    wl = WeisfeilerLehman()
    node_representations = wl.fit_transform(X, num_iterations=depth, initial_relabeling=initial_relabeling)
