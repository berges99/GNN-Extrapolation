...
def main():
    ...
    if args.embedding_scheme == 'WL':
        if args.method == 'continuous':
            formatted_dataset = fromNetworkx2Torch(networkx_dataset, add_degree=True) # TODO canviar per adaptar-ho al nou argument de tipus de initial labeling
    ...
    node_representations_kwargs = OrderedDict({
        k: v for k, v in args.ordered_args[node_representations_kwargs_idx1 + 1:node_representations_kwargs_idx2]
    })
    ...
    distance_kwargs = OrderedDict({
        k: v for k, v in args.ordered_args[node_representations_kwargs_idx2 + 1:]
    })
    distances_filename = \
